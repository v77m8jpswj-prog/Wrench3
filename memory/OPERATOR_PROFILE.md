...
- **WATCHLIST shipped** Feb 28 ~late evening. New `/watchlist` page + endpoints (`/api/scrape/watchlist`, `/api/scrape/watchlist/{id}/crawl`, `/api/scrape/watchlist/crawl-all`). Doc adds URLs Wrench should pull into the brain on demand. Uses existing scraper.py + library_chunks pipeline (zero new infra). Login-required sites auto-match Vault credentials. New mongo collection: `crawl_watchlist`. Doc added it because he was rightfully wondering why we hadn't built it already given scraper.py was sitting there. Home tile added next to LIBRARY.
